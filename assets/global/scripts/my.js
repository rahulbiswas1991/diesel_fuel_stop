...
$('.goUpLevel').click(function() {
    var currentid = $(this).attr('data-current');
    $.ajax({
        type: "POST",
        url: basepath + 'process/get_above_node',
        data: {
            'ref': currentid
        },
        dataType: 'json',
...
if (("#pxl-revinst-chartcontainer").length) {
    //$.cookie("revchart-type", 20, { expires : 10 });
    //Cookies.set('revchart-type', 20, { expires: 7 });
    var type = '';
...
/*$('.modeenabled').click(function(){
    toastr.clear();
    var mode = $(this).attr('data-type');
    if(mode==1)
    {
        $('#pay_mode').val(mode);
        $('#pay_mode_heading').text('Bitcoin');
        $('#pay_mode_img').attr('src',basepath+'assets/images/bitcoin-card.png');
        $('#pay_mode_btn').text('Bitcoin');
    }
    else if (mode==2)THIS IS MY CODE
    {
        $('#pay_mode').val(mode);
        $('#pay_mode_heading').text('Ethereum');
        $('#pay_mode_img').attr('src',basepath+'assets/images/et-card.png');
        $('#pay_mode_btn').text('Ethereum');
    }
    else
    {
        toastr.error('This mode is not valid selection');
    }
    $('#digitalpaymentpop').modal('show');
});*/
...
/*$("#add-account").on("hidden.bs.modal", function () {
   $('#addbank_delsfrm').bootstrapValidator('resetForm', true);
});*/
...
// vikash comomnet this its already in matherapp.js
    ...
    //                         if (data.status == 1) {
    //                             location.reload();
    //                             /*if(data.banks_count>=3)
    //                             {
    //                               $('.addbnkbtnon').attr('id','bank_disabledadd');
    //                             }
    //                             if(data.banks_count==1)
    //                             {
    //                               $('#all_bank_list').html('');
    //                               $('#bankvwbutton').text('view');
    //                             }

    //                             html += '<div class="inner-package-payment1" style="padding:15px;" id="bank_'+data.ref+'"><div class="row"><label class="custom-radio1"><input name="bank_radio" id="primbank'+data.ref+'" data-type="4" data-ref="'+data.ref+'" class="notprimary_bank" data-name="'+data.name+'" type="radio"><span class="checkmark"></span></label><div class="col-sm-2"><img src="'+basepath+'assets/images/bank1.png"></div><div class="col-sm-10 text-left"><h4 id="name_'+data.ref+'">'+data.name+'<a href="javascript:void(0);" class="pull-right delete_banks" dataref="'+data.ref+'"><img src="'+basepath+'assets/images/delete.png"></a></h4><p>Beneficiary Name: '+data.beneficry+'<br>Account Number: <span id="acc_'+data.ref+'">'+data.num+'</span><br>Bank Address: '+data.bradd+'<br>Swift Code: '+data.ifsc+'<br>Branch Code: '+data.brcode+'</p></div></div></div>';
    //                             $('#all_bank_list').prepend(html);
    //                             $('#bnk_pri_div').prepend('<label class="custom-radio"><input class="notprimary" data-type="4" id="primpay_4" name="radio" type="radio"><span class="checkmark"></span></label>');
    //                             toastr.success(data.message);*/
    //                             /*if(data.default==1)
    //                              {
    //                               $('#bnk_pri_div').addClass('inner-package-selected');
    //                               $('#bnk_pri_div').prepend('<label class="custom-radio"><input checked="checked" name="radio" type="radio"><span class="checkmark"></span></label>');
    //                              }
    //                              else
    //                             {
    //                               $('#bnk_pri_div').prepend('<label class="custom-radio"><input name="radio" type="radio"><span class="checkmark"></span></label>');
    //                             }*/
    ...
    $('#change_upass').bootstrapValidator({
            excluded: ':disabled',
            fields: {
                u_oldpass: {
                    validators: {
                    ...
                    validators: {
                        ...
                        callback: {
                            callback: function(value, validator, $field) {
                                var cpassword = $("input[name=u_cnewpass]").val();
                                if (value === '') {
                                    return true;
                                }
                                ...
                                if (cpassword != '') {
                                    if (value != cpassword) {
                                        $('#change_upass').bootstrapValidator('revalidateField', 'u_cnewpass');
                                        return true;
                                    } else {
                                        validator.updateStatus('u_cnewpass', 'VALID');
                                    }
                                }
                                return true;
                            }
                ...
                u_cnewpass: {
                    validators: {
                        ...
                        callback: {
                            ...
                            callback: function(value, validator, $field) {
                                var password = $("input[name=u_newpass]").val();
                                if (value === '') {
                                    return true;
                                }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        $(".updiv").remove();
                        $(".kyc_button").remove();
                        $("#aprejectline").text('Pending').removeClass('btn-danger btn-primary').addClass('btn-primary btn btn-circle');
                        $("#nidrejectline").text('Pending').removeClass('btn-danger btn-primary').addClass('btn-primary btn btn-circle');
                        $('#heading_kyc').text('KYC Documents');
                        toastr.success(data.message);
                    } else if (data.status == 0) {
                        toastr.error(data.message);
                    } else {
    ...
    $('#bit_addressform').bootstrapValidator({
            fields: {
                bit_address: {
                    validators: {
                        notEmpty: {
    ...
    //ends
    /*$('#mether_creditfm').bootstrapValidator({
            excluded: ':disabled',
            fields: {
                user_id: {
                    validators: {
        ...
        .on('success.form.bv', function(e){
            ...
            var transferamt =  $('#tranfer_amt').val();
            swal({
                title: 'Are you sure',
                text: "To Transfer "+transferamt+" Credits to "+transferuser+" ?",
                type: 'question',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        ...
                        /*if(data.type==1)
                                       {
                                         $('#bitcoincryp_div').html('');
                                         $('#bit_pri_div').children().first().remove();
                                         $('#bitcoincryp_div').html('<h4>Bitcoin</h4><a class="btn btn-outline blue-btn btn-circle cryptocard_popup" href="javascript:void(0);" data-type="1" data-name="Bitcoin" data-img="assets/images/bitcoin-card.png" data-holder="Enter Bitcoin">Add</a>');
                                       }
                                       else if(data.type==2)
                                       {
                                        $('#ethereumcryp_div').html('');
                                        $('#eth_pri_div').children().first().remove();
                                        $('#ethereumcryp_div').html('<h4>Ethereum</h4><a class="btn btn-outline blue-btn btn-circle cryptocard_popup" href="javascript:void(0);" data-type="2" data-name="Ethereum" data-img="assets/images/et-card.png" data-holder="Enter Ethereum">Add</a>');
                                       }
                                       else if(data.type==3)
                                       {
                                        $('#perfectmoney_div').html('');
                                        $('#per_pri_div').children().first().remove();
                                        $('#perfectmoney_div').html('<h4>Perfect Money</h4><a class="btn btn-outline blue-btn btn-circle cryptocard_popup" href="javascript:void(0);" data-type="3" data-name="Perfect Money" data-img="assets/images/perfekt-card.png" data-holder="Enter Perfect Money">Add</a>');
                                       }
                                       else
                                       {
...
$('.request_withdrawal_refund').click(function() {
    Swal.fire({
        title: 'Are you sure ?',
        text: "Do you want to cancel membership and withdraw amount.",
        type: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.value) {
            $.ajax({
                type: "POST",
                url: basepath + 'user/withdrawRefund',
                data: {
                    'action': 'wr'
                },
                dataType: 'json',
                success: function(data) {
                    ...
                    if (data.status == 1) {
                        Swal.fire({
                            title: data.message,
                            text: "Update your KYC documents and bank detail to get withdrawal amount.",
                            type: 'info',
                            allowOutsideClick: false
                        }).then((result) => {
                            if (result.value) {
