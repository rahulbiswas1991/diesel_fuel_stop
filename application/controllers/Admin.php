...
public function ManageLeadsByAgents($rowno = 0)
    {
        //Start Search Section
        $search['search_by'] = (isset($_GET['search_by'])) ? $_GET['search_by'] : '';
        $search['condition'] = (isset($_GET['condition'])) ? $_GET['condition'] : '';
...
//         }
			...
			$mode = 0;
				$u_info = $this->user_model->get_data('users', ' AND  email= "' . $email.'"', '*', 'full');
				if (count($u_info) > 0) {
						echo json_encode(array("status" => 0, "message" => "This Email is already exist.")); die;
				}else{
 ...
 //echo "<pre>"; print_r($roi_details['share_percentage']); die();
	...
	public function importfuelrecord_sheet(){
        ...
        // if(in_array($_FILES['file']['type'],$mimes)){
			...
			foreach($object->getWorksheetIterator() as $worksheet) // ITERATE ALL WORKSHEETS IN EXCEL FILE
			{
				...
				for($row=2; $row<=$highestRow; $row++)
				{
                    ...
                    if(count($checkdata) > 0){
                        // IF THIS RECORD IS AVAILABLE IN DTAABASE SKIP
                    }else{
						...
						if($fuel_gallons > 0){
                            $roi_details = $this->common_model->get_data("mannual_share_per", "AND status=1 AND user_id = $user_id order by id DESC","*", "1");
                         ...
                         $fuel[$i]['isactive'] = 1;
                         $fuel[$i]['created_date']= date('Y-m-d H:i:s');
                         $fuel[$i]['month'] = $worksheet->getTitle();
                         $i++;
